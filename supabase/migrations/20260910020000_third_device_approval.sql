-- A third device waits for an explicit Account Management approval. The
-- provider session is kept but has no ERP data access while it is pending.
drop function public.complete_login_attempt(uuid,text,uuid);
create function public.complete_login_attempt(p_ticket uuid,p_outcome text,p_session uuid default null) returns text
...
begin
 ...
 if p_outcome='invalid_credentials' then
   update public.profiles set failed_login_attempts=least(5,failed_login_attempts+1), status=case when failed_login_attempts>=4 then 'locked' else status end,
 ...
 if (select count(*) from public.device_sessions where profile_id=p.id and status='active') >= 2 then
   ...
   values('device_login','Account Management',p.id,'profile',p.id::text,jsonb_build_object('activeDeviceCount',2),jsonb_build_object('sessionId',p_session::text,'deviceLabel','ERP sign-in','startedAt',s.created_at),'Third-device sign-in approval required','pending',clock_timestamp()+interval '1 day');
...
create function public.decide_device_login(p_request bigint,p_approve boolean,p_reason text) returns void
language plpgsql security definer set search_path='' as $$
...
begin
 ...
 if p_approve then
   ...
   if old.id is not null then update public.device_sessions set status='logged_out',ended_at=clock_timestamp(),ended_reason='Approved third-device replacement' where id=old.id; end if;
 ...
 insert into public.notifications(recipient_id,notification_type,title,message,approval_request_id) values(r.requester_id,'approval',case when p_approve then 'Third-device sign-in approved' else 'Third-device sign-in rejected' end,p_reason,r.id);
 insert into public.audit_events(actor_id,actor_name,action,entity_type,entity_id,reason,before_data,after_data,approval_request_id) values(auth.uid(),(select employee_name from public.profiles where id=auth.uid()),case when p_approve then 'Third-device sign-in approved' else 'Third-device sign-in rejected' end,'approval_request',r.id::text,p_reason,r.current_data,r.proposed_data,r.id);
end; $$;
