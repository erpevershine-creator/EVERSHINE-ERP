...
grant select on ho to authenticated;
set local role authenticated;
select is((select count(*) from public.approval_requests where id=(select request2 from ho)),1::bigint,'successor can read selected pending request through RLS');
select is((select count(*) from public.approval_request_accounts where request_id=(select request2 from ho)),1::bigint,'successor can read affected-account snapshot through RLS');
select ok((select can_decide from public.request_decision_capabilities(array[(select request2 from ho)])),'UI capability recognizes selected temporary approval');
reset role;
...
set local role authenticated;
select is((select count(*) from public.approval_requests where id=(select request2 from ho)),0::bigint,'revoked successor cannot read selected request');
select is((select count(*) from public.handover_requests),0::bigint,'revoked successor cannot read handover records');
reset role;
...
set local role authenticated;
select is((select count(*) from public.approval_request_accounts where request_id=(select request2 from ho)),0::bigint,'expired assignment cannot expose affected accounts');
select is((select count(*) from public.request_decision_capabilities(array[(select request2 from ho)])),0::bigint,'expired assignment exposes no decision capability');
reset role;
