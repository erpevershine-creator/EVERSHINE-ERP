...
select throws_ok($$select public.finish_owner_recovery(operation,true) from fixture$$,'P0001','Provider outcome does not match committed receipt','recovery cannot complete before provider proof');
update auth.users set encrypted_password='synthetic-recovery-verifier',raw_app_meta_data=jsonb_build_object('erp_password_operation',(select operation from fixture)) where id=(select owner_id from fixture);
