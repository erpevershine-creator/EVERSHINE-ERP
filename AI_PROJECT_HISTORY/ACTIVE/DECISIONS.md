...
| D137 | Owner reports creating a real employee Admin account and requests Owner-managed per-employee additional permissions alongside ERP Role templates. Implement the Owner surface now, preserving D61/D66/D68/D69: grants are account-specific, audited with reason and before/after details, preserved across role-template updates, and have no expiry. Owner uses an explicit Approve & save action with a recorded self-approved decision. Removing an additional grant restores the account's base access; this slice does not deny base grants or authorize Admin-managed individual adjustments. Earlier delegated-Admin individual adjustment requirements remain future work. No actual grants for the employee are chosen by Codex. |
...
| D142 | Owner explicitly confirms erp.evershine@gmail.com as the main EVERSHINE ERP backup account and kyawthetaungftty@gmail.com as a separate newly created Owner-controlled Google account for system-managed recovery-key custody. Preserve D127: no manually entered Backup Recovery Key. Account selection is confirmed; recovery-account sign-in, separate protected credential binding, key protection/retrieval design and computer-loss recovery proof remain pending. This does not establish that Google account separation alone enforces ERP restore approval. No real key export or upload has occurred. |
| D143 | Owner asks to defer Main Backup account verification until the next day and continue the next professional Business-phase step. Preserve the local backup, Recovery account and existing offsite proof; do not repeat or invalidate Main OAuth consent during this pause. Business workflow rules remain subject to module-by-module confirmation under D16/D83/D86. |


## Supplier and commercial contract — conversation decisions, 2026-09-10

These decisions were previously present in the conversation but not persisted here. See docs/SUPPLIER-CONTRACT.md for the current contract, reference boundary and unresolved details. Requirements recorded does not mean implementation accepted.

| ID | Owner-confirmed decision |
| --- | --- |
| D144 | Complete the six-stage foundation/business goal: record a baseline and Owner checklist, close identity/security gaps, accept backup/DR independently, confirm module contracts, build Supplier/Product first locally, and use acceptance gates before subsequent modules. Preserve Owner data. Fast Mode and Codex Worktrees were requested; earlier attempts were rejected by automatic approval review for usage limits, so neither is claimed configured. |
| D145 | Supplier and Product are separate modules; navigation grouping comes after business pages. Supplier uses Legal Name only and SUP + country + independent five-digit country sequence, e.g. SUP-MM-00001. Generated codes cannot change or be reused. Same country plus normalized Legal Name blocks duplicate creation. No Country-change/code-regeneration workflow is requested. |
| D146 | Supplier has one Contact Person (Name/Phone required, Email optional) and one Address (Address Line, City/Township, State/Region, Country). Incomplete Draft is allowed; submission requires contact/address completeness. Tax ID / Company Registration Number is not required for this slice. |
| D147 | New Supplier details, one Commercial Agreement and one Financial Formula must be complete before a single Request Approval. Owner or an authorized Admin approves the complete package once; it becomes Active immediately together. The original Owner-only self-approval exception remains. Do not infer that a later assistant's blanket creator prohibition removed it. |
| D148 | One current Active Agreement and Formula each applies globally to the Supplier, without Account/Site/Warehouse override. One currency is selected using ISO 4217 code with full-name display. Changes require revision/approval; the old version remains current until approval and then becomes Archived history without deletion. Reason Note remains required. |
| D149 | Agreement controls Financial Titles and optional Tax/Value, selectable as Amount or Percentage. Formula controls Basis, +, -, *, / and sequence using Agreement titles. Do not reduce the design to a fixed Discount/Markup field or impose tax when no tax title is configured. Supplier price excludes tax. |
| D150 | Preserve six-decimal internal precision and two-decimal final Line Total / Invoice Total. The earlier MMK 0.5 rounding proposal is superseded. Detailed rounding/reconciliation examples remain part of the final contract acceptance. |
| D151 | Base Price × Quantity = Total; configured Title calculations, including any Tax, lead to Sub Total; Sub Total − Deposit = Grand Total. Deposit is after Tax and other title steps. Deposit source/bounds and actual payment posting are not established by this arithmetic decision. |
| D152 | Formula Sequence is configured while viewing Sample Calculation; the user adjusts the sequence as needed and reviews the result. Draft changes can be previewed; changes to an Active Formula follow the already-confirmed revision/approval process. No rigid default title order is imposed. |
| D153 | Owner confirms Deposit is the actual amount prepaid for each individual Purchase. Financial Formula takes that per-Purchase amount and subtracts it after all title calculations including Tax: Sub Total − Deposit = Grand Total. This is not a fixed Agreement value or a percentage deposit rule. Payment evidence/linking, allocation and handling of excess prepayment remain open; no payment/ledger posting is implemented by this confirmation. |
| D154 | Owner rejects accepting Deposit greater than Sub Total for the Purchase. Enforce Deposit <= Sub Total against the calculated tax-inclusive Sub Total; reject the invalid input rather than silently clamping it, converting excess into Supplier Advance or accepting a negative Grand Total caused by excess Deposit. Revalidate when calculation inputs change. This records the rule; runtime validation is not yet implemented. |

| D155 | Owner directs Foundation first: use an isolated worktree if feasible, organize execution by Plan/Goal and finish foundation controls/evidence before resuming Supplier questions. Deposit above Sub Total remains rejected. After foundation acceptance, review business modules and follow Supplier → Product → Purchase → Inventory/Pricing → Customer → Promotion → Order → Stock Survey → Return/Exchange → Delivery → Invoice → Finance → Statement → Reports → App User Guideline → further agreed scope. Review dependencies before implementation and ask remaining Supplier questions systematically. Fast mode is optional when useful. The preceding no-adjustment question remains unanswered and parked. |

## Confirmed delivery sequence; module details require confirmation
