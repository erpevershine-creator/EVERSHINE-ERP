...
| F2 Permissions/approval/handover | Delegated Admin scope, individual overrides/denies, reject/re-draft/expiry, exact approvals and handover/temporary responsibilities without changing original actors | IN PROGRESS — scoped handover is implemented for device approvals; other approval types still require the same request-level adapter |
