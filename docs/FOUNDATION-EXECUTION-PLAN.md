...
| F3 Foundation operation | Server pagination/search/filter/sort, governed exports, audit/notification consistency, settings and truthful usage monitoring; remove or clearly gate remaining sample-only behavior | IN PROGRESS — live settings/usage, server page ranges/counts and a local idempotent reminder worker are implemented; global search, governed server export and durable Production scheduling remain open |
