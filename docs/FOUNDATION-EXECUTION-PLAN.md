...
| --- | --- | --- |
| F0 Baseline | Exact source checkpoint, pending-file preservation, migration/hash manifest, dependency lock, fresh typecheck/lint/unit/build/DB results, roles/RLS/RPC/audit/backup evidence and Owner checklist | IN PROGRESS — candidate evidence is current at 278 isolated SQL assertions and eight Auth workflows; Owner freeze/acceptance is still pending |
| F1 Identity/security | Account/profile/password administration, provider-operation fencing/reconciliation, login throttling, two-device and third-device approval, seven-day idle reapproval, expiry/reminders; real bypass/role/concurrency probes on isolated fixtures | IN PROGRESS — admission, device, password fencing/provider receipts and recovery reconciliation pass; profile edit and expiry-reminder scheduler remain open |
| F2 Permissions/approval/handover | Delegated Admin scope, individual overrides/denies, reject/re-draft/expiry, exact approvals and handover/temporary responsibilities without changing original actors | PASS on isolated candidate controls — device and permission-template approvals use request-level handover; individual grants remain explicitly Owner-only and audited |
| F3 Foundation operation | Server pagination/search/filter/sort, governed exports, audit/notification consistency, settings and truthful usage monitoring; remove or clearly gate remaining sample-only behavior | IN PROGRESS — live settings/usage and server page ranges/counts are implemented; global search, server governed export and scheduler-backed reminders remain open |
...
| F5 Integrated verification | Source/SQL/server enforcement, relevant unit + database + isolated Auth/Storage + desktop/mobile workflow tests; negative/replay/stale/interruption probes, exact results and remaining external dependencies | OPEN |
