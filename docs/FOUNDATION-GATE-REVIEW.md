...
| Identity/security | Account/password editing, delegated scope, pagination, handover, two-device/third-device/idle controls, throttling, interrupted recovery reconciliation | PARTIAL: password authority/fencing, session admission, replay/throttle, two-device/third-device approval and seven-day idle reapproval tests pass; account profile editing, expiry reminders, handover and recovery reconciliation remain open |
...
| Runtime verification | Typecheck/lint/tests/build/database tests against exact frozen candidate | Current worktree: typecheck, lint, 28/28 Node tests, optimized build, 232/232 rollback pgTAP assertions and isolated schema-only 232/232 checks pass; DB lint has only two pre-existing unused-parameter warnings |
