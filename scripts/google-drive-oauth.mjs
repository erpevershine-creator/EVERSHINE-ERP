...
export function makeAuthorization(client, port) {
  ...
  url.search = new URLSearchParams({
    ...
    access_type: "offline",
    prompt: "consent",
    login_hint: backupGoogleAccount,
  }).toString();
  return { state, verifier, redirect_uri, url: url.toString() };
}
...
export async function verifyIdentity(accessToken, fetcher = fetch) {
  ...
  if (
    about.user?.emailAddress?.toLowerCase() !== backupGoogleAccount ||
    typeof about.user?.permissionId !== "string" ||
    !about.user.permissionId
  )
    throw Error("GOOGLE_ACCOUNT_MISMATCH");
  return { email: backupGoogleAccount, permissionId: about.user.permissionId };
}
export async function exchangeCode(
  ...
  const identity = await verifyIdentity(token.access_token, fetcher);
  return {
    ...identity,
    client_id: client.client_id,
...
export async function checkConnection(client, connection, fetcher = fetch) {
  if (
    connection.email !== backupGoogleAccount ||
